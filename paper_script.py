...
def extract_borders(piece, strip_width=16):
    ...
    top = lab[0:sw, :].astype(np.float32)
    bottom = lab[h-sw:h, :].astype(np.float32)
    left = lab[:, 0:sw].astype(np.float32)
    right = lab[:, w-sw:w].astype(np.float32)
...
def normalize_strip(s):
    arr = s.reshape(-1, 3).astype(np.float32)
...
def border_distance(stripA, stripB, sideA, sideB):
    ...
    d2 = np.power(np.sum(np.power(np.abs(a - b[::-1]), p)), exp)
    return min(d1, d2)
